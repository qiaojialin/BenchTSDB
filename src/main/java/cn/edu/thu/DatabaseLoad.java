...
public class DatabaseLoad {
  ...
  private static Logger logger = LoggerFactory.getLogger(DatabaseLoad.class);
...
//      }
    ...
    for (int threadId = 0; threadId < config.THREAD_NUM; threadId++) {
      Thread thread = new Thread(new DatabaseWriter(config, thread_files.get(threadId), statistics));
  ...
  static class DatabaseWriter implements Runnable {
    ...
    public DatabaseWriter(Config config, List<String> files, final Statistics statistics) {
      this.database = DatabaseFactory.getDatabaseManager(config);
      database.initClient();
      this.config = config;
