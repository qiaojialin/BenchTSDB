...
public class ORCManager implements IDataBaseManager {
  ...
  public long insertBatch(List<Record> records) {
    ...
    for (int i = 0; i < records.size(); i++) {
      ...
      device.setVal(i, record.tag.getBytes());
      String s = new String(device.vector[i], device.start[i],device.length[i]);
      ...
      Reader.Options readerOptions = new Reader.Options(conf)
              .searchArgument(
                      SearchArgumentFactory
                              .newBuilder()
                              .nullSafeEquals("device", PredicateLeaf.Type.STRING, tagValue)
...
//            continue;
//          }
          BytesColumnVector c = (BytesColumnVector)batch.cols[1];
          String device = c.toString(r);
          if(device.equals(tagValue)){
            double fieldValue = ((DoubleColumnVector) batch.cols[2]).vector[r];
//            System.out.println(result + " " + t + " " + device + " " + fieldValue);
            result++;
          }
